...
int32_t main(int argc,char** argv){
    ...
    repa(i, n) {
    	repa(j, 31) {
    		ll leftDiff = x[i] - (1 << j);
    		ll rightDiff = x[i] + (1 << j);

    		bool getLeft = binary_search(all(x), leftDiff);
    		bool getRight = binary_search(all(x), rightDiff);

    		if (getLeft && getRight)
    			 if (sz(ans) < 3)
    			 	 ans = vl({leftDiff, x[i], rightDiff});
