...
int32_t main(int argc,char** argv){
    ...
    repa(i, k) {
    	int n; cin >> n;
    	vl x(n); repa(i, n)  cin >> x[i];
    	a[i] = x;
    }
    
    unordered_map<ll, pair<int,int>> idx;
    bool chk = false;
    ...
    repa(i, k) {
       ...
       for (int m = 0; m < sz(a[i]); m++)
           a[i][m] = (sum - a[i][m]);	
       
       repa(m, sz(a[i])) {

	       if (idx.find(a[i][m]) == idx.end())
	           	   idx[a[i][m]] = make_pair(i, m);
	       else {
	           	   auto p = idx[a[i][m]];
	           	   if (p.first != i)  {

	           	   	  cout << "YES" << endl;
	           	   	  auto a = make_pair(p.first + 1, p.second + 1);
	           	   	  auto b = make_pair(i + 1, m + 1);
	                  
                      return cout << a.ff << " " << a.ss << " \n" << b.ff << " " << b.ss << endl, 0;
 	           	   	  chk = true;
	           	   	  break;
	           	   }
	       }   
