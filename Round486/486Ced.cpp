...
int32_t main(int argc,char** argv){
    ...
    int n; cin >> n;
    vector<pair<int, pair<int,int>>> a(n); repa(i, n) {
    	int s; cin >> s;
    	vi x(s); repa(i, s) cin >> x[i];

    	int sum = accumulate(all(x), 0);
    	repa(j, s) 
    	   a.eb(make_pair(sum, make_pair(i, j)));
    }   
    ...
    repa(i, sz(a) - 1) {
    	if (a[i].ff == a[i + 1].ff)
    		 if (a[i].ss.ff != a[i + 1].ss.ff)
    		 	 return cout << "YES" << endl << a[i].ss.ff << " " << a[i].ss.ss << endl << a[i + 1].ss.ff << " " << a[i + 1].ss.ff << endl, 0;
    }
