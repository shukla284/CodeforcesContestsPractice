...
int32_t main(int argc,char** argv){
    ...
    int a, m, n, x, p; cin >> a >> n >> m;
    vc(bool) rn(a + 1, false); repa(i, n) {
    	int l, r; cin >> l >> r;
    	rep(j, l + 1, r + 1)
    	   rn[j] = true;
    }
    ...
    vi wt(a + 1, numeric_limits<int>::max());
    repa(i, m)
       cin >> x >> p, wt[x] = min(wt[x], p);

    vl dp(a + 1, numeric_limits<int>::max()); dp[0] = 0;
    
    rep(i, 1, a + 1) {
    	if (rn[i]) {
    		rep(j, 0, i)
    		   if (wt[j] != numeric_limits<int>::max())
    		   	  dp[i] = min(dp[i], dp[j] + (i - j) * wt[j]);
    	}

    	else 
    		dp[i] = dp[i - 1];
    }
    cout << ((dp[a] == numeric_limits<int>::max()) ? -1 : dp[a]) << endl;
