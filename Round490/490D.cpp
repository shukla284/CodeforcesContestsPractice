...
int32_t main(int argc,char** argv){
    ...
    repa(i, m << 1) {
       int curr = (i % m);
       while (sz(rem[curr]) > k) {
       	  int pull = rem[curr].back();
       	  rem[curr].pop_back();
       	  pool.emplace_back(make_pair(pull, curr));
       }
       
       while (sz(rem[curr]) < k && !pool.empty()) {
       	  int elem = pool.back().ff;
       	  int rem_val = pool.back().ss;

       	  pool.pop_back();
       	  rem[curr].emplace_back(elem);
